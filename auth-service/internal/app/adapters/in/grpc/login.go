...
func (s *Server) Login(ctx context.Context, req *auth.LoginRequest) (*auth.LoginResponse, error) {
	if err := s.validator.Validate(req); err != nil {
		s.logger.ErrorContext(ctx, "validation error", "error", err.Error(), "email", req.GetEmail())
		return nil, errors.NewValidationError("invalid request: %v", err).
			WithDetails("email", req.GetEmail())
	}

	s.logger.InfoContext(ctx, "login request received", "email", req.GetEmail())
	...
	if err != nil {
		s.logger.ErrorContext(ctx, "login failed", "error", err.Error(), "email", req.GetEmail())
		return nil, err
	}

	s.logger.InfoContext(ctx, "login successful", "email", req.GetEmail())
