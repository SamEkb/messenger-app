...
func NewTokenRepository(q postgres.QueryEngine, logger logger.Logger) *TokenRepository {
	return &TokenRepository{q: q, logger: logger.With("component", "token_repository")}
}
...
func (r *TokenRepository) ValidateToken(ctx context.Context, token string) (bool, error) {
	...
	err := r.q.GetContext(ctx, &expiresAt, `
